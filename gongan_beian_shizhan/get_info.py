...
def into_url(domain_name):
    ...
    error = bowser.find_element_by_xpath('//*[@id="domainerror"]').text  # 验证码识别失败
    if error:
        # 清空验证码输入框并点击验证码刷新图片
        input_code.clear()
        click_img = bowser.find_element_by_xpath('//*[@id="domainform"]/div/div[2]/div/img')
        click_img.click()
        time.sleep(0.5)
        ...
        input_code.send_keys(soc_code())
        right = bowser.find_element_by_xpath('//*[@id="domainright"]/img')  # 验证码识别正确
        if right:
            # 点击查询按钮
            bowser.find_element_by_xpath('//*[@id="domainform"]/div/div[3]/div/button').click()
            try:
                try:
                    basic_info = bowser.find_element_by_xpath('/html/body/div[1]/div[3]/div[3]/div[1]/table')
                    ...
                    if bowser.find_element_by_xpath('//*[@id="a_wzym"]'):
                        info = url_all_info(
        ...
        try:
            try:
                ...
                if bowser.find_element_by_xpath('//*[@id="a_wzym"]').text:
                    info = url_all_info(
                        main_domain=domain_name.strip('\n'), url_name='', main_body='', url_type='', use_name='',
                        recode_number='', public_address='', recode_time='', is_code=False)
                    db.session.add(info)
